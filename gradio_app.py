...
def infer_from_ui(
    ...
    use_sample_s2=False,
    auto_fill_s2=False,
    # streaming toggle (False = normal blocking inference)
    stream_enabled=False,
    # When True, streaming yields will include raw PCM16 bytes as a fourth element
    stream_as_bytes: bool = False,
):
    try:
    ...
    try:
        ...
        if target_audio is None:
            if stream_enabled:
                # yield a single failure status for streaming mode
                yield "Inference produced no audio", None, MODEL_CACHE.get("model_path")
                return
            return "Inference produced no audio", None

        # If streaming is enabled, stream out incremental audio chunks to Gradio
        if stream_enabled:
            # chunk the audio into N roughly-equal parts and yield each as it's written to disk
            sr = 24000
            total_samples = target_audio.shape[1]
            # choose chunk size ~1s (or smaller for short audio)
            chunk_size = sr
            # make output dir
            os.makedirs(os.path.dirname(output_path), exist_ok=True)

            # initial status
            yield ("Streaming started", None, MODEL_CACHE.get("model_path"))
            ...
            for i in range(num_chunks):
                ...
                # accumulate into concat_so_far so each yielded audio contains prior parts
                if concat_so_far is None:
                    concat_so_far = part.clone()
                else:
                    concat_so_far = torch.cat([concat_so_far, part], dim=1)

                tmp_chunk = tempfile.NamedTemporaryFile(delete=False, suffix=f".part{i}.wav")
                # write wav file for backward compatibility
                sf.write(tmp_chunk.name, concat_so_far.cpu().squeeze(0).numpy(), sr)
                ...
                # yield either (status, path, model_path) or (status, path, model_path, pcm_bytes)
                if stream_as_bytes:
                    yield (f"Streaming chunk {i+1}/{num_chunks}", tmp_chunk.name, MODEL_CACHE.get("model_path"), pcm_bytes)
                else:
                    yield (f"Streaming chunk {i+1}/{num_chunks}", tmp_chunk.name, MODEL_CACHE.get("model_path"))

            # finally, write the final output_path and yield done
            ...
            if stream_as_bytes:
                yield (status_msg, output_path, MODEL_CACHE.get("model_path"), final_pcm_bytes)
            else:
                yield (status_msg, output_path, MODEL_CACHE.get("model_path"))
            return

        # non-streaming flow follows existing behavior
        # Save output
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
def main(stream_default: bool = False):
    with gr.Blocks() as demo:
        ...
        with gr.Tabs():
            with gr.TabItem("Single Speaker"):
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        stream_checkbox = gr.Checkbox(label="Enable streaming (yield partial outputs)", value=stream_default)
            ...
            with gr.TabItem("Dual Speaker"):
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        stream_checkbox_d = gr.Checkbox(label="Enable streaming (yield partial outputs)", value=stream_default)
            ...
            with gr.TabItem("Streaming"):
                with gr.Row():
                    with gr.Column():
                        json_file_s = gr.File(label="Upload podcast JSON (optional) — follows example/podcast_script schema")
                        manual_text_s = gr.Textbox(label="Manual utterance (used when no JSON) — speaker tag [S1] will be added if omitted", lines=4, placeholder="Hello, this is a test.")
                        # reuse sample_files from earlier
                        sample_choice_s = gr.Dropdown(
                            label="Choose a sample reference (from repo)",
                            choices=sample_files,
                            value=(sample_files[0] if sample_files else None),
                        )
                        use_sample_s = gr.Checkbox(label="Use selected sample as reference audio (if no upload)", value=True)
                        prompt_wav_s = gr.Audio(label="Or upload your own reference audio (takes precedence)", type="filepath")
                        prompt_text_s = gr.Textbox(label="Reference text (optional) — short transcript describing voice/style")
                        use_dialect_prompt_s = gr.Checkbox(label="Enable dialect prompt (use dialect-specific model)", value=False)
                        dialect_prompt_text_s = gr.Textbox(label="Dialect prompt (e.g. <|Henan|>我今天很高兴)", placeholder="<|Henan|>...", lines=2)

                    with gr.Column():
                        model_choice_s = gr.Dropdown(label="Select a local model (from pretrained_models)", choices=model_dirs, value=default_model)
                        model_path_s = gr.Textbox(label="Local model directory (override)", value=(os.path.join("pretrained_models", default_model) if default_model else "pretrained_models/SoulX-Podcast-1.7B-dialect"))
                        output_path_s = gr.Textbox(label="Output WAV file (will be overwritten)", value="outputs/output_stream.wav")
                        llm_engine_s = gr.Dropdown(label="LM engine (hf = HuggingFace, vllm = vLLM)", choices=llm_choices, value=llm_default)
                        fp16_flow_s = gr.Checkbox(label="Use FP16 for flow model (recommended on supported GPUs)", value=True)
                        seed_s = gr.Number(label="Random seed (int)", value=42, precision=0)
                        # hidden state used to force streaming=True in the infer call
                        stream_true_state = gr.State(value=True)
                        run_stream_btn = gr.Button("Run Streaming Inference")
                        status_s = gr.Textbox(label="Job status / messages", interactive=False)
                        out_audio_s = gr.Textbox(label="Generated audio file (path)", interactive=False)
                        model_status_s = gr.Textbox(label="Model status (loaded path)", interactive=False)

                        # Hidden/default state values for the streaming-tab invocation
                        speaker2_prompt_wav_state_s = gr.State(value=None)
                        speaker2_prompt_text_state_s = gr.State(value="")
                        dialogue_text_state_s = gr.State(value="")
                ...
                run_stream_btn.click(
                    ...
                    inputs=[
                        ...
                        use_sample_s,
                        prompt_text_s,
                        speaker2_prompt_wav_state_s,
                        speaker2_prompt_text_state_s,
                        dialogue_text_state_s,
                        use_dialect_prompt_s,
                        dialect_prompt_text_s,
                        model_choice_s,
                        model_path_s,
                        output_path_s,
                        llm_engine_s,
                    ...
                    outputs=[status_s, out_audio_s, model_status_s],
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stream", action="store_true", help="Enable streaming mode by default in the UI")
    parser.add_argument("--port", type=int, default=7860, help="Gradio server port")
    args = parser.parse_args()

    # pass streaming default into main so the checkbox is pre-set
    main(stream_default=args.stream)
